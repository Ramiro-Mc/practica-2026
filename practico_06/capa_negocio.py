...
class NegocioSocio(object):
    ...
    def regla_1(self, socio):
        ...
        if self.datos.buscar_dni(socio.dni) is not None:
            raise DniRepetido(f"El DNI {socio.dni} ya existe")
        return True
    ...
    def regla_2(self, socio):
        ...
        if len(socio.nombre) <= self.MIN_CARACTERES or len(socio.nombre) >= self.MAX_CARACTERES:
            raise LongitudInvalida(f"El nombre debe tener entre {self.MIN_CARACTERES + 1} y {self.MAX_CARACTERES - 1} caracteres")
        if len(socio.apellido) <= self.MIN_CARACTERES or len(socio.apellido) >= self.MAX_CARACTERES:
            raise LongitudInvalida(f"El apellido debe tener entre {self.MIN_CARACTERES + 1} y {self.MAX_CARACTERES - 1} caracteres")
        return True
    ...
    def regla_3(self):
        ...
        if self.datos.contarSocios() >= self.MAX_SOCIOS:
            raise MaximoAlcanzado(f"Se alcanzó el máximo de {self.MAX_SOCIOS} socios")
        return True
